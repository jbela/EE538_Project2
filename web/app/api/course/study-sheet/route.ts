...
import { postGenerateStudySheet } from "@/lib/summarize-backend";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await postGenerateStudySheet({ corpus, courseTitle: courseLabel });
    return NextResponse.json(result);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Study sheet generation failed";
